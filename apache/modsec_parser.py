...
RULES_PATH = "/etc/apache2/modsecurity.d/owasp-modsecurity-crs/rules/"
requestfiles   = glob.glob("%s/REQUEST-*.conf" % RULES_PATH)
responsefiles  = glob.glob("%s/RESPONSE-*.conf" % RULES_PATH)
...
for rule_file in allfiles:
    if ('EXCLUSION' in rule_file or 'CORRELATION' in rule_file or 'BLOCKING-EVALUATION' in rule_file):
        continue
    print "Looking at file %s" % rule_file
                    ...
                    if (len(line_data) > 2): #validation check
                        for key in line_data:
                            ...
                            if (append_line == ""):
                                ...
                                append_line += "%smatched_vars%s" % (SEPARATOR, SEPARATOR) + "%{MATCHED_VARS}" + "%smatched_vars%s" % (SEPARATOR, SEPARATOR)
                                append_line += "%srule_file%s%s%srule_file%s" % (SEPARATOR, SEPARATOR, rule_file, SEPARATOR, SEPARATOR)
                                append_line += "%s" % END_OF_RULE
